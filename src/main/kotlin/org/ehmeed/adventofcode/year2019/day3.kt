...
private fun main() {
    val closestCrossingPointDistance = Day3.getClosestCrossingPointManhattanDistance(wirePaths)
    println("Closest point crossing distance is $closestCrossingPointDistance")

    val closestCombinedSteps = Day3.getCombinedStepsToClosesIntersection(wirePaths)
    println("Closes combined steps is $closestCombinedSteps")
}

object Day3 {
    fun getClosestCrossingPointManhattanDistance(wirePaths: String): Int {
        val wires = wirePaths.split("\n")
            .mapIndexed { index, encodedWire -> Wire(index + 1, encodedWire) }

        val startingPoint = Cell(0, 0)

        val (wire1, wire2) = wires
            .asSequence()
            .map { it.getOccupiedCells(listOf(startingPoint)).distinct() }
            .toList()

        return wire1.intersect(wire2)
            .filter { it != startingPoint }
            .map { it manhattanDistanceTo startingPoint }
            .drop(1)
            .min() ?: throw IllegalStateException("Given wires do not cross")
    }

    fun getCombinedStepsToClosesIntersection(wirePaths: String): Int {
        val wires = wirePaths.split("\n")
            .mapIndexed { index, encodedWire -> Wire(index + 1, encodedWire) }
        ...
        val (wire1, wire2) = wires
            .asSequence()
            .map { it.getOccupiedCells(listOf(startingPoint)).distinct() }
            .map { cells -> cells.mapIndexed { index, cell -> index - 1 to cell } }
            .toList()

        return (wire1 + wire2).asSequence()
            .groupBy { it.second }
            .filter { it.value.size >= 2 }
            .map { it.value.sumBy { v -> v.first } }
            .drop(2)
            .min() ?: throw IllegalStateException("Given wires do not cross")
    }
}

private infix fun Cell.manhattanDistanceTo(other: Cell): Int = (abs(x) - abs(other.x)) + (abs(y) - abs(other.y))

private fun Wire.getOccupiedCells(visitedCells: List<Cell>): List<Cell> =
    this.moves.fold(visitedCells) { visited, move ->
        val currentCell = visited.last()
        val newlyVisited: List<Cell> = when (move.direction) {
            Direction.UP -> (currentCell.y + 1..currentCell.y + move.distance).map { Cell(currentCell.x, it) }
            Direction.DOWN -> (currentCell.y + 1 downTo currentCell.y - move.distance).map { Cell(currentCell.x, it) }
            Direction.RIGHT -> (currentCell.x + 1..currentCell.x + move.distance).map { Cell(it, currentCell.y) }
            Direction.LEFT -> (currentCell.x + 1 downTo currentCell.x - move.distance).map { Cell(it, currentCell.y) }
        }
...
private data class Wire(val id: WireId, val moves: List<Move>)

private inline class WireId(val value: Int)

private fun Wire(id: Int, encodedMoves: String): Wire = Wire(WireId(id), encodedMoves.split(",").map { Move(it) })

private data class Move(val direction: Direction, val distance: Int)

private fun Move(encodedMove: String): Move = Move(Direction(encodedMove[0]), encodedMove.substring(1).toInt())

enum class Direction {
    UP, DOWN, LEFT, RIGHT;
}

private fun Direction(direction: Char): Direction = when (direction) {
    'U' -> Direction.UP
    'D' -> Direction.DOWN
    'L' -> Direction.LEFT
    'R' -> Direction.RIGHT
